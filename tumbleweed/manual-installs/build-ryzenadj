#!/bin/bash
# Build and install ryzenadj for Fedora Linux and Fedora-derived distributions
# Based on AUR PKGBUILD: https://aur.archlinux.org/packages/ryzenadj
# Official Fedora build deps confirmed at: https://github.com/FlyGoat/RyzenAdj
...
readonly REQUIRED_PACKAGES=("gcc" "gcc-c++" "make" "cmake" "pciutils-devel")
...
# Check for Fedora or a Fedora-derived distribution
...
if [ "${ID}" != "fedora" ] && [[ "${OS_ID_LIKE}" != *"fedora"* ]]; then
    echo -e "${RED}ERROR:${NC} This script requires Fedora Linux or a Fedora-derived distribution." >&2
    echo -e "${RED}ERROR:${NC} Detected: ${PRETTY_NAME} (ID=${ID})" >&2
    echo -e "${RED}ERROR:${NC} Supported distributions include: Fedora, RHEL, CentOS Stream," >&2
    echo -e "${RED}ERROR:${NC}   AlmaLinux, Rocky Linux, Nobara, Ultramarine, Bazzite, and" >&2
    echo -e "${RED}ERROR:${NC}   any distribution where ID_LIKE includes 'fedora'." >&2
    exit 1
fi

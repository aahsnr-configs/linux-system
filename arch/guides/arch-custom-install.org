...
- =/dev/nvme0n1p1= - _2GB_, type: EFI System (ESP - needs space for
...
** Test Snapshot Functionality \*** Test Automatic Snapshots
#+begin_src sh
...
** Step-by-Step Chroot Process \*** Step 1: Boot into Arch Live Environment
...
** Common Maintenance Tasks \*** Regenerate Initramfs
#+begin_src sh
...
** Exiting the Chroot Environment \*** Step 1: Exit Chroot
#+begin_src sh
...
** Troubleshooting Chroot Issues \*** Can't Find LUKS Device
#+begin_src sh
...
_During chroot/recovery:_ The directories already exist in the @
...
_Key advantages:_ - Dynamic management (no need to restart) - Zone-based
...
** Installation \*** Install Firewalld
#+begin_src sh
...
1. _drop_ - Drops all incoming packets without reply. Only outgoing
   connections are allowed.

2. _block_ - Rejects all incoming connections with icmp-host-prohibited
   message.

3. _public_ (default) - For use in public areas. You don't trust other
   computers. Only selected incoming connections are accepted. Default
   allows SSH and DHCPv6-client.

4. _external_ - For use on external networks with masquerading (NAT)
   enabled. Only selected incoming connections are accepted.

5. _dmz_ - For computers in your DMZ that have limited access to your
   internal network. Only selected incoming connections are accepted.

6. _work_ - For use at work where you trust most computers. More
   services may be allowed.

7. _home_ - For use at home where you trust most devices. More services
   may be allowed.

8. _internal_ - For use on internal networks when you trust the other
   computers. More services are allowed.

9. _trusted_ - All network connections are accepted. Complete trust.
...
** Basic Configuration \*** Configure Default Zone
#+begin_src sh
...
** Common Service Configurations \*** Web Server
#+begin_src sh
...
:END: \*** 6.1 Interface-Specific Zones
:PROPERTIES:
...
:END: \*** 8.1 Panic Mode (Emergency Lockdown)
:PROPERTIES:
...
:END: \*** 9.1 Minimal Desktop Configuration
:PROPERTIES:
...
** Part 11: Integration with Other Tools \*** Docker/Podman Integration
...
** Part 12: Backup and Restore \*** Backup Configuration
#+begin_src sh
...
_Can't connect to service after opening port:_
...
** Part 14: Performance and Optimization \*** Configuration File
...
1. _Default deny policy_ - Only allow what's needed
2. _Use specific zones_ - Don't put everything in ‘public’
3. _Regular audits_ - Review rules monthly
4. _Least privilege_ - Only open ports that must be accessible
5. _Source restrictions_ - Use rich rules to limit by IP when possible
6. _Rate limiting_ - Use rich rules with limits for brute-force
   protection
7. _Logging_ - Enable for security-critical services
8. _Backup_ - Keep configuration backups
9. _Test changes_ - Test in runtime before making permanent
10. _Integration_ - Combine with fail2ban for additional protection

...

{
  ...
  # --- 5. ACTIVATION SCRIPT: KILL BAKED-IN AI EXTENSIONS ---
  # VSCodium sometimes inherits bundled AI extensions from upstream VS Code binaries.
  # This safely forces them to be disabled via CLI without breaking the Nix store link.
  home.activation.disableVscodiumAI = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    VSCODIUM_BIN="${pkgs.vscodium}/bin/codium"
    if [ -f "$VSCODIUM_BIN" ]; then
      $VSCODIUM_BIN --disable-extension GitHub.copilot 2>/dev/null || true
      $VSCODIUM_BIN --disable-extension GitHub.copilot-chat 2>/dev/null || true
      $VSCODIUM_BIN --disable-extension ms-vscode.vscode-copilot 2>/dev/null || true
      rm -rf ~/.config/VSCodium/CachedExtensionVSIXs 2>/dev/null || true
      rm -rf ~/.config/VSCodium/CachedData 2>/dev/null || true
    fi
  '';
}

I'll research the latest Doom Emacs documentation and conventions before translating your config. Let me search for the most current information.I have enough information now. Let me write the three Doom Emacs configuration files. I have a thorough understanding of:

1. **Doom's literate config module** (`:config literate`) — tangles `config.org` to `config.el` by default; you can target other files using `:tangle packages.el`
2. **`init.el` rules** — must contain the `doom!` block, loaded before tangling
3. **`packages.el` rules** — uses `package!` macro, isolated environment, no `use-package!` or `after!` allowed
4. **`config.el` conventions** — uses `use-package!`, `after!`, `add-hook!`, `setq!`, `map!`, `add-load-path!`, `defadvice!`
5. **Modules to enable** based on your vanilla config

Now I'll write the three files. The strategy is:

- `init.el` enables all relevant Doom modules
- `config.org` is the literate source with most blocks tangling to `config.el` and one section tangling to `packages.el`
- `packages.el` declares third-party packages not already provided by Doom

---

# Doom Emacs Translation

Here's a quick map of how your vanilla config maps onto Doom modules per Doom's module system in $DOOMDIR/init.el. Many things you wrote by hand (evil, evil-collection, evil-surround, doom-themes, doom-modeline, vertico+marginalia+consult+embark+orderless, corfu+cape, magit+forge+diff-hl, treemacs, pdf-tools, eglot+eglot-booster, flymake, apheleia, treesit+treesit-fold, dirvish, helpful, popper, projectile, snippets, etc.) are already bundled into Doom modules — so we delete those bespoke `use-package` blocks and just enable the module + configure on top with `after!` / `use-package!`.
...
(doom! :input
       ...
       :completion
       (corfu +icons +orderless +dabbrev)   ; modern completion-at-point
       (vertico +icons)                     ; vertical minibuffer, consult, embark, marginalia, orderless

       :ui
       doom                                 ; doom-themes + general aesthetic defaults
       doom-dashboard                       ; splash screen
       hl-todo                              ; highlight TODO/FIXME/NOTE/HACK/...
       (ligatures +extra)
       indent-guides                        ; indent-bars-ish guides
       modeline                             ; doom-modeline
       nav-flash                            ; flash cursor on big jumps
       ophints                              ; highlight operator regions
       (popup +all +defaults)               ; window rule manager (your `popper` analog)
       (treemacs +lsp)
       unicode
       (vc-gutter +pretty +diff-hl)         ; diff-hl in the fringe
       vi-tilde-fringe                      ; tildes past EOB (replaces your ar/vi-tilde)
       window-select
       ...
       electric
       (ibuffer +icons)
       (undo +undo-fu)                      ; undo-fu + undo-fu-session
       vc
       ...
       :checkers
       (syntax +flymake)                    ; flymake (Doom defaults to flycheck; we choose flymake)
       (spell +jinx)                        ; jinx instead of flyspell
       ...
       (eval +overlay)
       (lookup +dictionary)
       (lsp +eglot)                         ; eglot + eglot-booster + consult-eglot
       magit                                ; magit + forge + magit-todos
       pdf                                  ; pdf-tools + saveplace-pdf-view + org-noter (+ org-pdftools via packages.el)
       tree-sitter                          ; treesit + treesit-fold + evil-textobj-tree-sitter
       ...
       markdown
       (nix +tree-sitter +lsp)
       (org +pretty +noter +pomodoro +journal +habit)
       (plantuml)
       ...
       :config
       literate                             ; tangle ~/.config/doom/config.org -> config.el
...
;;; $DOOMDIR/packages.el
;;
;; Declarative package list. Run 'doom sync' after editing.
;; Only `package!`, `disable-packages!`, `unpin!` and straight customization
...
## Key translation decisions & caveats

<details>
<summary><b>What's NOT in the translation (and why)</b></summary>

- **straight.el bootstrap** — Doom manages straight itself; you never bootstrap it from your config.
- **early-init.el GC tuning** — Doom ships its own carefully tuned early-init; overriding it can regress startup performance. Use `gcmh` tuning in `config.org` instead.
- **`reload-init-file`, manual tangle functions** — Use `doom/reload` (bound to `SPC h r r`) or `M-x doom/reload-config`.
- **`perspective.el` + integrations** — Replaced by `:ui workspaces` (persp-mode-based). It already integrates with project.el, treemacs, consult.
- **`popper`** — Replaced by `:ui popup` which is more powerful and Doom-integrated.
- **Vanilla `evil` use-package block** — entirely subsumed by `:editor evil +everywhere`.
- **Vanilla `vertico`/`marginalia`/`consult`/`embark`/`orderless`/`corfu`/`cape`/`nerd-icons-corfu`/`nerd-icons-completion` blocks** — provided by the two `:completion` modules.
- **Vanilla `magit`/`forge`/`diff-hl`/`magit-todos`** — provided by `:tools magit` + `:ui vc-gutter +diff-hl`.
- **Custom transient menus** — Doom's leader-key bindings replace them; keep them if you want by adding them back inside `(after! transient ...)` blocks.
- **`general.el` config** — Replaced by Doom's built-in `map!` macro (powered by general.el under the hood).
- **`helpful`, `which-key`, `recentf`, `saveplace`, `savehist`** — All built-in to Doom defaults.

</details>

<details>
<summary><b>Things that work differently in Doom</b></summary>

- `SPC` is the leader (default Doom binding scheme). Your full `general.el` `ar/global-leader` setup is largely redundant — Doom already provides `SPC f f`, `SPC b b`, `SPC w v`, `SPC g s`, etc.
- `K` over a symbol shows docs; `gd` jumps to definition.
- Run `doom sync` after every change to `init.el` or `packages.el`.
- Doom uses `flycheck` by default for `:checkers syntax`; we override to `+flymake` because your config is flymake-based.
- `:editor format +onsave` enables apheleia on save automatically.

</details>

<details>
<summary><b>Manual follow-ups after first run</b></summary>

1. `M-x nerd-icons-install-fonts` if you haven't already.
2. Run `M-x treesit-install-language-grammar` for each language you actually use (or call them on first open of each file type).
3. `doom doctor` to check missing system dependencies (clangd, basedpyright, nixd, texlab, bash-language-server, marksman, yaml-language-server, alejandra, ruff, prettier, plantuml, etc.).
4. Verify `lisp/org-src-context.el` is present in `~/.config/doom/lisp/`.

</details>

This should give you a faithful, close-enough Doom Emacs port of your vanilla config with all your custom logic (project org contexts, GTD, Denote integration, org-src-context, LaTeX workflow) intact, while letting Doom handle the 80% of plumbing that's identical between your config and Doom's defaults.

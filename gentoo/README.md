...
### 3.2 — Wipe Existing Metadata (and optionally TRIM the SSDs)
...
- [ ] **DONE**
      The LUKS header (~4 MiB per container) stores cipher parameters and encrypted master keys. **If the header is corrupted, all data is permanently lost.** Back up each header to offline storage immediately (see Part 16 for TPM recovery key generation).
...
rm -R "/mnt/gentoo/etc/portage/package.env"
cp -R "$HOME/Git/configs/linux-system/gentoo/preconfig/package.env /mnt/gentoo/etc/portage/"

cp -R "$HOME/Git/configs/linux-system/gentoo/preconfig/env/ /mnt/gentoo/etc/portage/"

cp -R "$HOME/Git/configs/linux-system/gentoo/preconfig/profile/ /mnt/gentoo/etc/portage/"
```
...
## 7 — Base Configuration and Portage

### 7.1 — Sync Portage and Select Profile
...
### 7.4 — Enable CachyOS‑Kernels Overlay
...
### 7.5 — Enable GPG Commit Verification for Portage
...
###  7.6 — Root Password and Administrative User
...
## 8 — Kernel: CachyOS‑Sources with Clang + kCFI

### 8.1 — Install Kernel Sources and Required Packages
...
### 8.2 — Configure the Kernel (Hardened, Minimal, Hardware‑Specific)
...
#### 8.2.2 — Apply the Kernel’s Built‑in Hardening Fragment
...
#   3. Calls the post‑install hook (Part 11.7) to re‑sign the UKI with sbctl
make install LLVM=1
```
...
| Generate UKI           | `dracut` (Part 11) | Creates a single signed `.efi` binary containing kernel + initramfs + cmdline     |
| Install UKI            | `installkernel`    | Places the UKI at `/efi/EFI/Linux/` where the UEFI firmware can load it           |
| Re‑sign UKI            | `sbctl` (hook)     | The post‑install hook from Part 11.7 runs `sbctl sign -s` on every UKI on the ESP |
...
#### 8.3.1 — Verify the Kernel Was Built with Hardening Options
...
### 8.4 — Generate Secure Boot Keys (Before First Boot)

- [ ] **TODO**

Dracut requires the Secure Boot signing keys to exist at the paths specified in its configuration (Part 11.1). Generate them now:
...
> **Note:** Key **enrollment** into UEFI firmware happens later (Part 14), after the first reboot into Setup Mode. For now, having the keys generated is sufficient for dracut to produce a signed UKI.
...
### 9.1 — Kernel Configuration for NVIDIA
...
### 9.2 — Kernel Command Line and Modesetting
...
- **`modules-sign`** — critical for Secure Boot. Its role is elaborated in section 9.4.
...
### 9.4 — Secure Boot and Module Signing
...
1. **Ensure Keys Exist** — the `sbctl` keys are at `/var/lib/sbctl/keys/db/db.key` and `/var/lib/sbctl/keys/db/db.pem` (generated in Part 8.4).
...
### 9.6 — Module Parameters and Blacklisting
...
The TPM is used for LUKS PCR‑sealing at boot (Part 15) and for SSH key storage (Part 21). There is no direct integration between the TPM and the NVIDIA driver. The driver’s security on this system is ensured through module signing (Secure Boot) and confinement via AppArmor.

### 9.8 — Rebuild the Initramfs and UKI
...
### 9.9 — AppArmor Integration for NVIDIA
...
### 9.10 — Hardening the NVIDIA Persistence Daemon with `svc-harden.py`

- [ ] **TODO**

The `nvidia-persistenced` service can be hardened using `svc-harden.py` (Part 26):
...
### 9.11 — Post‑Install Verification
...
# These must exist before dracut runs (see Part 8.4).
...
### 11.2 — Kernel Command Line (Embedded in the UKI)
...
- `quiet` — suppresses non‑critical kernel messages at boot; the `loglevel=3` parameter previously included was redundant because `quiet` already sets the console log level to 4 (KERN_WARNING and below are suppressed). Remove any separate `loglevel=` parameter.
...
- `intel_iommu=on iommu=force` — IOMMU strict mode for DMA protection (Part 27).
...
- `audit=1` — enables the audit subsystem required by `auditd` (Part 26).
...
### 11.4 — Running System Crypttab (noauto)
...
### 11.6 — Automatic Re‑Signing After Kernel Updates
...
## 14 — Final System Setup and First Boot

### 14.1 — User Account (already configured)

The root password was set and locked, and the `ahsan` user was created in Section 7.6. No further action is needed here.
...
### 14.2 — Enable Essential Services

- [ ] **TODO**

```bash
systemctl enable NetworkManager sshd apparmor firewalld auditd snapper-timeline.timer snapper-cleanup.timer snapper-boot.timer systemd-resolved dnscrypt-proxy cockpit.socket
```

### 14.3 — Regenerate UKI (First Time Manually)
...
### 14.5 — Secure Boot Key Enrollment (After First Boot)
...
### 14.6 — Verification (After First Boot)
...
### 15.2 — PCR Register Justification
...
## 16 — TPM 2.0 Deep Dive: SSH Keys, Diagnostics, FIDO2 & PCR Predictions

Your TPM 2.0 chip is already sealing your LUKS keys (Part 15). This section covers the rest of its capabilities: storing SSH private keys inside the TPM, useful diagnostic commands, virtual FIDO2 tokens, and the emerging `systemd‑pcrlock` framework for automatic PCR prediction after firmware updates.

### 16.1 — TPM Fundamentals (Recap)
...
The `systemd‑cryptenroll` enrollment you performed in Part 15 uses the TPM’s sealing capability: the LUKS master key is wrapped (encrypted) by the TPM’s Storage Root Key, and the TPM will only unwrap it if the current PCR values match those recorded during enrollment.

### 16.2 — TPM‑Backed SSH Keys
...
This has already been partially implemented in Part 21 of your guide. The `app‑crypt/tpm2‑pkcs11` package provides the PKCS#11 library that bridges OpenSSH and the TPM. The key creation and SSH configuration is unchanged from Part 21; the material below adds **context** and explains how to **verify** the key is hardware‑backed.

#### 16.2.1 — Understanding the Architecture
...
#### 16.2.2 — The `tss` Group and User Access

- [ ] **TODO**

For unprivileged users to access the TPM, they must be members of the `tss` group. This was done in Section 7.6 when you created the `ahsan` user. The Gentoo wiki explicitly documents this requirement.
...
#### 16.2.3 — Key Hierarchy Inside the TPM

When you run `tpm2_ptool init` (Part 21.2), the following hierarchy is created:
...
#### 16.2.4 — Empty PIN vs. PIN‑Protected Keys
...
#### 16.2.5 — Verifying the Key is Hardware‑Backed
...
#### 16.2.6 — Using TPM Keys with Git Commit Signing

- [ ] **TODO**

The TPM key can also sign Git commits. The Gentoo wiki documents this workflow using the SSH‑agent protocol already configured in Part 21.6.
...
Compare the output with the PCR table in Part 15.2. After a UEFI firmware update, PCR[0] will change. After enrolling new Secure Boot keys, PCR[7] will change.

#### 16.3.2 — Listing Persistent Keys
...
#### 16.3.3 — Checking the TPM Version and Capabilities
...
#### 16.3.4 — Reading the TPM Event Log
...
### 16.4 — TPM as a Virtual FIDO2 Token
...
### 16.5 — TPM‑Backed PIN for sudo and Login (Experimental)
...
### 16.7 — TPM‑Assisted Random Number Generation
...
### 16.8 — TPM‑Backed GPG Keys (Experimental)
...
### 16.10 — Summary of TPM Integration

| Feature                                   | Guide Section  | Status                        |
| ----------------------------------------- | -------------- | ----------------------------- |
| LUKS2 unlock via TPM2+PIN                 | Part 15        | ✅ Fully configured           |
| LUKS recovery key enrollment              | Part 15        | ✅ Configured                 |
| TPM PCR sealing (0+2+7+12)                | Part 15.2      | ✅ Configured                 |
| TPM‑backed SSH keys                       | Part 21        | ✅ Configured                 |
| TPM SSH key Git signing                   | Section 16.2.6 | ✅ Optional                   |
| TPM diagnostic tools (`tpm2‑tools`)       | Section 16.3   | Recommended                   |
| TPM virtual FIDO2 (`tpm‑fido`)            | Section 16.4   | ⚠️ Experimental / from‑source |
| TPM‑backed sudo PIN (`pinpam`)            | Section 16.5   | ⚠️ Experimental / from‑source |
| TPM‑backed GPG keys                       | Section 16.8   | ⚠️ Experimental               |
| `systemd‑pcrlock`                         | Section 16.6   | ⚠️ Experimental               |
| Manual TPM recovery after firmware update | Part 30        | ✅ Configured                 |

All of the above is verified against the Gentoo Wiki (specifically the Trusted Platform Module/SSH page, which documents `tpm2‑pkcs11` installation and `PKCS11Provider` configuration), the systemd man pages (`systemd‑pcrlock(8)` confirms the prediction and NV‑Index policy mechanism), and the `systemd‑cryptenroll(1)` manual page.

---

## 17 — `systemd-homed`: Per‑User Encrypted Home Directories
...
All of the above are verified against the Gentoo wiki’s `systemd-homed` article.

### 17.2 — Installation and USE Flags
...
### 17.5 — Creating a `systemd-homed` User

> **This section assumes the `ahsan` user already exists (created in Section 7.6) and has data in `/home/ahsan`. If you have not yet rebooted into the installed system, complete Part 14 first, then return here.**

There is no `homectl convert` command. Migrating an existing traditional user to `systemd-homed` is a manual process following the upstream conversion guide. The steps below follow that guide, adapted for this system.
...
### 17.6 — PAM Integration and Automatic Locking on Suspend

For `systemd-homed` to work with SSH, the display manager (SDDM), and `sudo`, the PAM stack must load `pam_systemd_home.so`. Gentoo’s `sys-auth/pambase` already includes this module when the `homed` USE flag is enabled (Section 17.2), so no manual PAM edits are required for basic functionality.
...
> **SSH note**: SSH public‑key authentication via TPM‑backed keys (Part 21) continues to work. The `PKCS11Provider` is independent of the user’s login password. After a suspend‑and‑resume, the `ssh-agent` socket may persist, but the home directory must be re‑activated before any files inside it can be accessed.
...
### 17.7 — Interaction with the Existing Hardening Setup
...
| **TPM2 LUKS unlock (Part 15)** | Unaffected — `systemd-homed` manages user home directories, not the root filesystem. You may also enroll a TPM2 or FIDO2 token on the homed container via `homectl update ahsan --fido2-device=auto` or `--tpm2-device=auto`.                                                                       |
...
| **`svc-harden.py` (Part 26)**  | Run `sudo svc-harden.py profile systemd-homed.service` after the service has been active for a week to profile its runtime behaviour and apply confinement directives.                                                                                                                              |
...
### 17.9 — Recovery and Troubleshooting
...
- Kernel lacks required options (`CONFIG_BLK_DEV_LOOP`, `CONFIG_DM_CRYPT`; see Section 17.1).
- The filesystem inside the LUKS container is marked dirty. Follow the Gentoo wiki’s repair procedure to mount and `fsck` the filesystem manually.
- The `nsswitch.conf` has not been updated (Section 17.3); tools cannot resolve the user.
...
## 21 — PAM and Authentication Hardening
...
> **Note on `pam_umask.so`:** As of April 2026, `pam_umask.so` is not yet part of the default Gentoo `pambase` package (tracked in Bug 938574). To set a default umask, add `session optional pam_umask.so umask=0027` to `/etc/pam.d/system-login` in the `session` block.
...
# SSH uses key‑only auth (Part 22); these requirements apply to
...
## 22 — SSH Hardening with TPM‑Backed Keys
...
### 22.1 — Install Required Packages
...
### 22.2 — Create the TPM‑Backed SSH Key (as the user, **not root**)
...
### 22.4 — SSH Client Configuration (system‑wide)
...
### 22.5 — Complete `/etc/ssh/ssh_config` (Hardened Client)
...
### 22.6 — ssh-agent Integration (Optional)
...
### 22.7 — Hardened sshd Configuration (Server)
...
### 22.8 — TPM‑Based Key Usage Cheat Sheet
...
- **Secure Boot**: **ENABLED** with custom keys (Part 14)

### Kernel Parameters (already embedded in UKI cmdline in Part 11)
...
### 24.1 — Installation and Kernel Setup
...
Ensure the following kernel options are enabled. The cachyos-sources `.config` sets most of these already; verify with `make menuconfig` in Part 8:
...
These are already embedded in the UKI cmdline (Part 11). No `lsm=` parameter is needed or should be added.
...
### 24.2 — AppArmor Parser Configuration
...
> **UKI note**: `early_policy=yes` requires the `apparmor` dracut module. Add `apparmor` to the `add_dracutmodules+=` line in `/etc/dracut.conf.d/00-base.conf` (already done in Part 11.1).

---

### 24.3 — apparmor.d Integration
...
### 24.4 — Configure Personal Directories
...
### 24.5 — Testing in Complain Mode and Switching to Enforce
...
### 24.6 — Recommended Enforce/Complain Mode Assignments
...
### 24.7 — Handling Profile Conflicts
...
## 25 — Confining Development Tools and Editors with AppArmor

This section covers how to extend AppArmor confinement to editors, interpreters, build tools, and package managers within a development environment. It assumes the `apparmor.d` profile set has already been installed (Part 24) and provides practical, tested workflows.

### 25.1 — Understanding the `apparmor.d` Confinement Model
...
### 25.2 — Profile Availability for Development Tools
...
| `python3` | ✅ Yes             | `apparmor.d/profiles-m-r/` | Confines the interpreter itself; see §25.3 for scripting limitations |
| `npm`     | ✅ Yes             | `apparmor.d/profiles-m-r/` | Covers the Node.js package manager                                   |
| `emacs`   | ❌ No              | —                          | Must be generated from scratch; see §25.6                            |
| `neovim`  | ❌ No              | —                          | Must be generated from scratch; see §25.5                            |
...
### 25.4 — Workflow: Deploying Existing Profiles
...
### 25.5 — Creating a Profile for Neovim
...
### 25.6 — The Challenge of Confining Emacs
...
### 25.7 — Confining Language‑Specific Package Managers
...
### 25.9 — Practical Integration into the Workflow
...
### 26.4 — Enable augenrules and Load Rules
...
## 27 — Hardened Firewalld Configuration

- [ ] **TODO**

This `firewalld` configuration controls network‑level traffic with a strict default‑drop policy.
...
## 28 — Application‑Layer Firewall: OpenSnitch
...
### 28.5 — Hardening OpenSnitch Itself

- [ ] **TODO**

Run `svc-harden.py` on the daemon (Part 31):
...
Generate an AppArmor profile with `aa-genprof` (Part 24):
...
### 28.6 — Interaction with Existing Hardening
...
## 29 — Host‑Based Intrusion Detection: AIDE and Rootkit Scanning

Your guide already implements several detection controls: `auditd` (Part 26), AppArmor (Part 24), and the Portage supply‑chain auditing pipeline (Part 32). A dedicated Host‑Based Intrusion Detection System (HIDS) adds file‑integrity monitoring (cryptographic checksums), rootkit detection, and active response. The table below evaluates the leading options for a standalone APT‑hardened workstation.
...
**Conclusion:** Deploy **AIDE** for cryptographic file‑integrity monitoring and both **rkhunter** + **chkrootkit** for lightweight rootkit detection. A full SIEM (Wazuh) is not needed on a single workstation.

### 29.1 — AIDE: File Integrity Monitoring
...
#### 29.1.1 — Installation and Initial Baseline
...
# (Part 26.3) plus additional critical paths.  Ensure the following are
...
#### 29.1.2 — Periodic Integrity Checks
...
#### 29.1.3 — Automated Weekly Scan

Extend the weekly security scan script from Part 32:
...
### 29.2 — Rootkit Detection: `rkhunter` and `chkrootkit`
...
#### 29.2.2 — Post‑Kernel‑Update Hook
...
## 30 — Cockpit Integration (Optional)
...
> **AppArmor:** No profile shipped by apparmor.d; bound to localhost only, and hardened with `svc-harden.py apply cockpit` (Part 31).
...
### 31.3 – Detailed Usage Walkthrough
...
#### 31.3.2 – `analyze` – See What Can Be Improved
...
#### 31.3.3 – `apply` – Interactive, Manual Hardening
...
#### 31.3.4 – `profile` – Auto‑Hardening via SHH (Behaviour‑Based)
...
#### 31.3.5 – `test` – Validate That the Hardening Didn’t Break Anything
...
#### 31.3.6 – `revert` – Roll Back to the Original State
...
#### 31.3.7 – `bisect` – Find the Culprit Directive
...
### 31.4 – Recommended Services for Hardening
...
### 31.5 – Learning Python with This Script
...
### 32.1 — Portage ELOG: Build and Post‑Install Logging
...
### 32.2 — Repository Integrity Verification
...
#### 32.2.1 — Git Commit Signature Verification

Already configured in Section 7.5. The `repos.conf` entry for `::gentoo` includes:
...
#### 32.2.2 — Full‑Tree Manifest Verification with Gemato
...
### 32.3 — Package Transaction Audit Logging
...
### 32.4 — GLSA Vulnerability Scanning
...
### 32.5 — Log Rotation for the Audit Log
...
### 32.6 — Weekly Security Automation

A systemd timer runs GLSA scanning, repository integrity verification, and reports results to the monitoring pipeline (Part 31).

#### 32.6.1 — Weekly Security Scan Script
...
#### 32.6.2 — Weekly Timer and Service Units
...
## 33 — Ongoing Monitoring, Log Review, and Vulnerability Alerting

### 33.1 — Mail Relay with msmtp
...
### 33.2 — Daily Auditd Summary

The script below extracts key metrics from the audit log and emails a summary. All audit keys reference the hardened ruleset deployed in Part 26. Recipient addresses and alert thresholds are defined at the top of the script so they can be changed without modifying complex shell logic.
...
# exist in the Gentoo-adapted rules (Part 26). Searching by message type captures
...
### 33.3 — Weekly Security Report

The weekly GLSA scan and repository integrity check are performed by `/usr/local/bin/weekly-security-scan.sh`, which was already deployed in Part 32. That script already contains its own systemd timer and emails its results directly. No additional wrapper is needed. To verify:
...
### 33.4 — Weekly AppArmor Denial Digest
...
### 33.5 — Systemd Timers for Automated Reports
...
# 2. Enroll TPM2+PIN (see Part 15 for full commands)
...
If you need to re‑enter the installed system from a live environment (e.g., for boot repair, UKI regeneration, or `snapper rollback`), the procedure below remounts the entire LUKS‑LVM‑Btrfs stack independently of the running kernel’s fstab. **All commands assume you have booted from a Gentoo Live‑DVD/USB** (or any rescue environment with `cryptsetup`, `lvm2`, and `btrfs‑progs`).

### 37.1 — Open LUKS and Activate LVM
...
> **Why no `subvol=`?** When the `subvol=` option is omitted, Btrfs mounts the _current default_ subvolume (the one set by `btrfs subvolume set-default`). This is the same behaviour that the running system’s fstab uses, and it means you never accidentally mount a stale snapshot.

### 37.3 — Mount All Subvolumes and the ESP
...
> **About `BTRFS_NOCOW`:** These subvolumes had CoW disabled at creation time via `chattr +C` (Part 5). The `BTRFS_NOCOW` variable simply omits `compress=zstd:1` because compression is incompatible with `nodatacow`, but it does **not** pass a `nodatacow` mount option — that is handled at the attribute level.

### 37.4 — Bind‑Mount Pseudo‑Filesystems
...
> **Why `--make-rslave`?** When `systemd` later unmounts something inside the chroot (e.g. a stale `/sys/fs/cgroup`), `rslave` prevents that unmount from propagating backward to the live environment’s `/sys`, which would otherwise tear down the host’s cgroup hierarchy. This is the standard Gentoo Handbook recommendation.

### 37.5 — Copy DNS Configuration and Enter the Chroot
...
> **Alternative:** If your rescue ISO ships `arch‑chroot` (Arch‑based media), you can use `arch‑chroot /mnt/gentoo` instead of the manual `chroot` command. It automatically handles `/dev/shm`, `/run`, and `resolv.conf`.

### 37.6 — Perform Recovery Tasks
...
> **`umount -l` (lazy unmount)** is used for safety; it detaches the filesystem immediately but finishes pending operations in the background. This avoids “target is busy” errors if a process (e.g. `systemd-journald` from the chroot) still holds a file descriptor.

### 37.8 — Quick Reference: Post-Install Chroot Script

For convenience, here is the entire chroot re-entry sequence as a single copy-pasteable block (assuming you are in a live environment):
...
When the TPM refuses to unseal, the initramfs falls back to prompting for a passphrase. This is the moment you reach for the recovery key that you generated during enrollment (Part 15) and stored securely offline.

---

### 38.1 — Why This Process Must Remain Manual
...
### 38.2 — Recovery Procedure

#### Step 1: Boot with the Recovery Key

When the TPM fails to unseal the LUKS key, the `systemd‑cryptsetup` initramfs service prompts for a passphrase. Enter the recovery key that you generated and stored offline during Part 15.
...
> **If you lost the recovery key**, there is no way to recover the data. The LUKS header contains no back‑door. This is why Part 3.7 insists on an offline header backup and Part 15 insists on storing the recovery key in at least two physically‑separate locations (paper + encrypted USB, safe‑deposit box + home safe).
...
### 38.3 — The `tpm‑re‑enroll` Convenience Script
...
### 38.4 — What Happens During Re‑Enrollment
...
**Immediate action:** Boot using the recovery key (as in Section 38.2, Step 1).
...
### 38.6 — PCR Justification (Repeated from Part 15)
...
### 38.7 — Automated Alternative: `systemd‑pcrlock` (Experimental)

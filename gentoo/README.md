...
- CIS AlmaLinux OS 10 Benchmark v1.0.0 (2025‑09‑30) — Section 4.1 (Firewalld)
...
**Target System:** Gentoo Linux · CachyOS‑sources kernel (Clang + ThinLTO + kCFI) · UKI direct UEFI boot · Secure Boot with custom keys · TPM2 + PIN LUKS2 (Argon2id) · AppArmor (apparmor.d, ~1500 profiles) · systemd‑homed per‑user LUKS2 · Btrfs with Tumbleweed‑style subvolume layout · Firewalld (nftables backend)

---

## Foreword

No official CIS Benchmark or DISA STIG exists for Gentoo Linux.  This document fills that gap by translating every recommendation from three separate benchmarks into concrete, verifiable steps for the hardened Gentoo workstation described in the accompanying `README.md`.

Three documents are merged here:
...
- **CIS Ubuntu 24.04 LTS STIG Benchmark** — the United States Department of Defense overlay, with CAT I (high severity), CAT II (medium severity), and CAT III (low severity) categories.
- **CIS AlmaLinux OS 10 Benchmark** — incorporated specifically for Section 4 (Host‑Based Firewall), which provides native firewalld‑specific audit and remediation procedures rather than the UFW‑oriented guidance in the Ubuntu benchmarks.

Where the STIG is stricter than the general CIS recommendation, the STIG requirement takes precedence in this guide unless a deliberate deviation is justified and documented (see §8 — Cryptographic Requirements).  Every recommendation is traceable to its source document through the CIS recommendation number.

---

## How to Use This Guide

Each recommendation is a self‑contained block containing:
...
2. **Profile & Status** — Which CIS levels and STIG categories apply, plus a quick icon:
   ...
   - ❌ **INTENTIONAL DEVIATION** — Deliberately not applied, with rationale documented.
   - 🔵 **NOT APPLICABLE** — Not relevant to a standalone workstation.
3. **What's Required** — A concise, plain‑English description of the requirement, taken directly from the source benchmark.
4. **Gentoo Implementation** — How the target system meets (or intentionally deviates from) the requirement, with specific references to relevant `README.md` sections (e.g., "README Part 16").
5. **Audit** — Copy‑paste‑ready commands to verify compliance.  A successful audit produces no output or the expected value; anything else warrants investigation.  Where the AlmaLinux benchmark provides audit scripts, those scripts have been adapted for the Gentoo environment but preserve the original logic.
6. **Remediation** — Copy‑paste‑ready commands to bring the system into compliance.  Only shown when the default configuration does not already satisfy the requirement.  Remediation commands are adapted for Gentoo's package manager (`emerge`), init system (`systemd`), and filesystem layout.

> **Convention:** Prompts beginning with `#` denote commands run as root.  Prompts beginning with `$` denote commands run as an unprivileged user.  File paths reflect the Gentoo filesystem hierarchy (e.g., `/lib64/security/` for PAM modules on amd64).
...
The CIS benchmarks require that unused filesystem kernel modules be prevented from loading to reduce the local attack surface.  The hardened Gentoo system blacklists these modules via `/etc/modprobe.d/blacklist-hardening.conf` (README Part 16).  Each entry uses both `install <module> /bin/true` (to intercept any attempt to load the module) and `blacklist <module>` (to prevent automatic loading by the kernel module loader).  After modifying the blacklist, the initramfs must be rebuilt with `dracut --force --regenerate-all` so that the blacklist takes effect in early boot.

---

##### 1.1.1.1 Ensure cramfs kernel module is not available

*STIG: UBTU‑24‑100030 (analogous principle)*

**Profile:** CIS Level 1 – Server & Workstation | **Status:** ✅ COMPLIANT

**What's Required**
The `cramfs` kernel module must be prevented from loading.  `cramfs` is a compressed read‑only Linux filesystem embedded in small‑footprint systems; it has known vulnerabilities and no legitimate use on a modern workstation.

**Gentoo Implementation**
`cramfs` is blacklisted in `/etc/modprobe.d/blacklist-hardening.conf` (README Part 16).  The configuration uses both `install cramfs /bin/true` and `blacklist cramfs`.
...
**What's Required**
The `freevxfs` (free Veritas VxFS) module must be blocked.  This is a legacy filesystem with no modern use on Linux workstations.
...
**What's Required**
The `hfs` (original Macintosh Hierarchical File System) module must be blocked.
...
**Audit**
```bash
modprobe -n -v hfs 2>&1 | grep -q 'install /bin/true' && \
  echo "✅ hfs is blocked" || echo "❌ hfs is NOT blocked"
```
...
**What's Required**
The `hfsplus` (HFS+, modern Macintosh filesystem) module must be blocked.  Exception: comment out the blacklist if macOS‑formatted drives must be connected.
...
**What's Required**
The `jffs2` (Journaling Flash File System 2) module must be blocked.  Multiple CVEs were published against jffs2 in 2025 (CVE‑2025‑38194, CVE‑2025‑38328).
...
**Audit**
```bash
modprobe -n -v jffs2 2>&1 | grep -q 'install /bin/true' && \
  echo "✅ jffs2 is blocked" || echo "❌ jffs2 is NOT blocked"
```
...
**What's Required**
The `overlay` (OverlayFS) kernel module must be blocked.  This is a Level 2 recommendation because containers (Docker, Podman, Kubernetes) depend on overlayfs.  OverlayFS has known CVEs: CVE‑2023‑32629, CVE‑2023‑2640, and CVE‑2023‑0386.
...
**What's Required**
The `squashfs` (compressed read‑only filesystem) module must be blocked.  Snap packages and Flatpak applications depend on squashfs.  Multiple CVEs were published in 2025–2026 (CVE‑2025‑38415, CVE‑2025‑40049, CVE‑2025‑40200).

**Gentoo Implementation**
Conditionally blacklisted in `blacklist-hardening.conf` (README Part 16).

**Audit**
```bash
modprobe -n -v squashfs 2>&1 | grep -q 'install /bin/true' && \
  echo "✅ squashfs blocked" || echo "⚠️ squashfs is loadable (Snap/Flatpak may need it)"
```

**Remediation (if neither Snap nor Flatpak are used)**
```bash
...
**What's Required**
The `udf` (Universal Disk Format, used for DVDs and Blu‑ray) module must be blocked.  Microsoft Azure requires udf; it should not be disabled on Azure‑hosted systems.
...
**Audit**
```bash
modprobe -n -v udf 2>&1 | grep -q 'install /bin/true' && \
  echo "✅ udf blocked" || echo "⚠️ udf is loadable"
```

**Remediation (if optical drives are not used)**
```bash
...
**What's Required**
The `firewire-core` (IEEE 1394) kernel module must be blocked.  FireWire provides DMA access, which is a well‑known vector for memory extraction attacks.

**Gentoo Implementation**
Both `firewire-core`, `firewire-ohci`, and `firewire-sbp2` are blacklisted in `blacklist-hardening.conf` (README Part 16).

**Audit**
```bash
modprobe -n -v firewire-core 2>&1 | grep -q 'install /bin/true' && \
  echo "✅ firewire-core is blocked" || echo "❌ firewire-core is NOT blocked"
```
...
**Gentoo Implementation**
Conditionally blacklisted in `blacklist-hardening.conf` (README Part 16).  The comment states that USB storage may be "required for recovery USB boot."  For a workstation that must satisfy the STIG profile, this should be fully blocked.
...
**What's Required**
The system administrator must review all loaded filesystem modules and disable any that are not required for the system's function.  The CIS benchmark identifies modules with known CVEs: `ceph` (CVE‑2022‑0670), `cifs` (CVE‑2022‑29869), `exfat` (CVE‑2022‑29973), `ext` (CVE‑2022‑1184), `fat` (CVE‑2022‑22043), `fscache` (CVE‑2022‑3630), `fuse` (CVE‑2023‑0386), `gfs2` (CVE‑2023‑3212), `nfs_common` (CVE‑2023‑6660), `nfsd` (CVE‑2022‑43945), and `smbfs_common` (CVE‑2022‑2585).

**Gentoo Implementation**
README Part 16 blacklists cover the most common attack vectors.  The administrator must manually review any additional loaded filesystem modules.
...
**Remediation**
For any module identified as unnecessary, follow the pattern established in 1.1.1.1–1.1.1.10: create a `.conf` file in `/etc/modprobe.d/` with `install <module> /bin/true` and `blacklist <module>`, then rebuild the initramfs with `dracut --force --regenerate-all`.

---

#### 1.1.2 Configure Filesystem Partitions

The target Gentoo system uses **Btrfs subvolumes with appropriate mount options** rather than separate physical partitions for each directory.  Btrfs subvolumes provide equivalent isolation — each subvolume can be snapshotted independently, and Btrfs honours mount options identically to a separate partition.  The system also uses **tmpfs** for `/tmp` and `/dev/shm` with the required restrictive mount options.

All seven CIS partition requirements are satisfied by the layout described in README Part 4 (Btrfs subvolume creation) and Part 11 (fstab).  The following table maps each CIS requirement to the Gentoo implementation:

| Directory | Required Mount Options | Gentoo Implementation |
|-----------|----------------------|----------------------|
| `/tmp` | `nosuid,nodev,noexec` | tmpfs mounted via fstab with `nosuid,nodev,noexec,size=2G` |
| `/dev/shm` | `nosuid,nodev,noexec` | tmpfs with `nosuid,nodev,noexec` |
| `/home` | `nosuid,nodev` (separate partition at Level 2) | `subvol=@/home` on Btrfs + systemd‑homed per‑user LUKS2 encryption |
| `/var` | `nosuid,nodev` (separate partition at Level 2) | `subvol=@/var` (CoW disabled via `chattr +C`) |
...
```bash
#!/bin/bash
# Verify all critical directories have the correct mount options
for mp in /tmp /dev/shm /home /var /var/tmp /var/log /var/log/audit; do
    echo "=== $mp ==="
    findmnt -kn "$mp" 2>/dev/null || echo "  NOT MOUNTED — check fstab"
done
```

Each directory should be mounted (no "NOT MOUNTED" messages), and the output for each should include the required mount options from the table above.

**Remediation (if a directory is not mounted or has incorrect options)**
Edit `/etc/fstab` to add or correct the mount entry, then run `mount -o remount <directory>` or reboot.  For example, to ensure `/tmp` is a tmpfs with the correct options:

```bash
# Add to /etc/fstab if not present:
echo "tmpfs /tmp tmpfs defaults,nosuid,nodev,noexec,size=2G 0 0" >> /etc/fstab
mount -o remount /tmp
```
...
Patch management procedures vary widely between enterprises.  The CIS benchmarks require that a patch management process be defined and maintained, that GPG keys be configured for package verification, and that repositories be correctly configured.  On Gentoo, these requirements are satisfied through Portage's built‑in mechanisms.
...
**What's Required**
The package manager must verify package integrity using GPG key signing.  Each repository should have a `gpgkey` URL pointing to the location of the GPG key.

**Gentoo Implementation**
Gentoo uses Portage with Git commit signature verification (README Part 5.3 and 21.2).  The `repos.conf` entry for `::gentoo` includes `sync-git-verify-commit-signature = yes` and references the Gentoo release OpenPGP key at `/usr/share/openpgp-keys/gentoo-release.asc`.  Additionally, `app-portage/gemato` provides full‑tree Manifest verification, recursively verifying the entire repository tree against the top‑level Manifest which is itself OpenPGP‑signed.
...
# Verify the Gentoo release key is present
ls -l /usr/share/openpgp-keys/gentoo-release.asc

# Verify gemato is installed
qpkg -I gemato && echo "✅ gemato installed" || echo "⚠️ gemato not installed"
```
...
**What's Required**
The `gpgcheck` option must be globally enabled so that RPM package signatures are always verified before installation.  On Gentoo, the equivalent is Manifest verification, which is enabled by default in Portage.

**Gentoo Implementation**
Portage verifies all Manifest checksums for every ebuild by default.  The `FEATURES="strict"` flag (set by the hardened profile) causes Portage to refuse any package whose Manifest cannot be verified.  No additional configuration is needed.

**Audit**
```bash
grep -E '^FEATURES' /etc/portage/make.conf | grep -q 'strict' && \
  echo "✅ Manifest verification (strict) is active" || echo "❌ strict not in FEATURES"
```

**Remediation**
```bash
# Add 'strict' to FEATURES in /etc/portage/make.conf if not present
sed -i 's/^FEATURES="\(.*\)"/FEATURES="\1 strict"/' /etc/portage/make.conf
# If FEATURES line does not exist:
echo 'FEATURES="strict"' >> /etc/portage/make.conf
```
...
**What's Required**
The `repo_gpgcheck` option (repository metadata signature verification) must be globally enabled.  Not all repositories support this feature; the administrator must research which repositories are configured and which support `repo_gpgcheck`.

**Gentoo Implementation**
Portage's Git sync mechanism with commit signature verification (see 1.2.1.1) provides equivalent integrity guarantees for the repository metadata.  The `gemato` tool performs full‑tree verification.  For overlay repositories that use rsync rather than Git, the administrator should verify that the overlay's Manifest is signed.

**Audit**
```bash
# List all configured repositories
portageq repos_config / | grep -E '^(location|sync-type|sync-uri)'

# For each overlay, check if gemato verification passes
gemato verify -K /usr/share/openpgp-keys/gentoo-release.asc \
  "$(portageq get_repo_path / gentoo)" 2>&1
```
...
**What's Required**
Package manager repositories must be correctly configured so that the system can receive the latest patches and updates.

**Gentoo Implementation**
The `::gentoo` repository is configured for Git sync with commit signature verification (README Part 5.3).  Additional overlays (GURU, CachyOS‑kernels, hyproverlay, xarblu‑overlay, pentoo) are enabled via `eselect repository`.

**Audit**
```bash
eselect repository list -i
# Verify that ::gentoo is present and that only approved overlays are enabled
```

**Remediation**
```bash
# Enable the main Gentoo repository
eselect repository remove gentoo 2>/dev/null || true
...
**What's Required**
Weak dependencies (package recommendations and suggestions) should not be automatically installed unless the system specifically requires the additional capabilities they provide.  On RPM‑based distributions, this is controlled by `install_weak_deps=false` in `/etc/dnf/dnf.conf`.

**Gentoo Implementation**
Portage does not have an equivalent of RPM weak dependencies.  The `--oneshot` and `--select` emerge options, combined with the `--depclean` action, give the administrator fine‑grained control over which packages are installed.  The hardened profile's `USE` flag settings already minimise the dependency tree.  No additional configuration is needed.

**Audit**
```bash
# Review the world file for any unintended packages
...
**What's Required**
The system must receive regular security updates.  Operating system updates should be performed through automated patch management on a monthly (or more frequent) basis.

**Gentoo Implementation**
Gentoo is a rolling‑release distribution.  As long as `emerge --sync` runs regularly, the system receives continuous security updates.  The weekly GLSA scan (`glsa-check` from `app-portage/gentoolkit`) identifies any packages affected by published Gentoo Linux Security Advisories (README Part 21.4).  This scan is automated via a systemd timer.
...
AppArmor is the Mandatory Access Control (MAC) system on the target Gentoo workstation.  It is implemented as a Linux Security Module (LSM) and is loaded early in the boot process via the UKI command line.  The `apparmor.d` project provides approximately 1500 profiles that confine system services, daemons, and user applications.  Profiles are installed in complain mode first, tested for a minimum of one week, and then switched to enforce mode.

This section maps the CIS and STIG requirements for MAC (which assume SELinux on Red Hat derivatives and AppArmor on Ubuntu) to the AppArmor configuration on the hardened Gentoo system.
...
**What's Required**
The AppArmor userspace tools (`apparmor`, `apparmor-utils`) must be installed.  The STIG also requires `apparmor‑utils` for profile management.

**Gentoo Implementation**
`sys-apps/apparmor`, `sys-apps/apparmor-utils`, and `sec-policy/apparmor-profiles` are emerged.  The `apparmor.d` profile set (~1500 profiles) is installed from the upstream Git repository (README Part 14.1).  The PAM AppArmor module is present at `/lib64/security/pam_apparmor.so`.

**Audit**
```bash
qpkg -I apparmor && echo "✅ AppArmor installed" || echo "❌ AppArmor NOT installed"
qpkg -I apparmor-utils && echo "✅ apparmor-utils installed" || echo "❌ apparmor-utils NOT installed"
...
**What's Required**
AppArmor must be activated at boot time via `apparmor=1` and `security=apparmor` kernel parameters.  The CIS benchmark assumes GRUB; the STIG requires the AppArmor service to be enabled and active.

**Gentoo Implementation**
The target system does not use GRUB.  Instead, the UKI (Unified Kernel Image) embeds `apparmor=1 security=apparmor` in its command line (README Part 8.2).  The kernel is compiled with `CONFIG_LSM="lockdown,yama,apparmor,bpf"` — the Gentoo‑recommended approach that makes the `lsm=` kernel command‑line parameter unnecessary.  Secure Boot prevents tampering with the UKI.  The `apparmor.service` is enabled and active.
...
**Gentoo Implementation**
Approximately 1500 profiles from the `apparmor.d` project are loaded (README Part 14.3).  Critical system profiles (systemd, sshd, dbus, polkit, firewalld, NetworkManager) are in enforce mode.  User‑application profiles are in complain mode pending site‑specific tuning.  The `apparmor.d` project strongly recommends a minimum one‑week testing period in complain mode before enforcing.
...
**Gentoo Implementation**
Critical system profiles are in enforce mode.  Some user‑application profiles remain in complain mode pending site‑specific tuning (README Part 14.6).  The `apparmor.d` project recommends a minimum one‑week testing period in complain mode before enforcing.
...
##### 1.3.1.5 Ensure the MCS Translation Service (mcstrans) is not installed (Automated)

**Profile:** CIS Level 1 – Server & Workstation | **Status:** ✅ COMPLIANT

**What's Required**
The `mcstrans` daemon (SELinux MCS Translation Service) must not be installed.

**Gentoo Implementation**
This is an SELinux‑specific package and is not present on an AppArmor‑based Gentoo system.

**Audit**
```bash
qpkg -I mcstrans 2>/dev/null && echo "❌ mcstrans installed" || echo "✅ mcstrans not installed"
```

---

##### 1.3.1.6 Ensure SETroubleshoot is not installed (Automated)

**Profile:** CIS Level 1 – Server | **Status:** ✅ COMPLIANT

**What's Required**
The `setroubleshoot` service (SELinux troubleshooting daemon) must not be installed.
...
**Audit**
```bash
qpkg -I setroubleshoot 2>/dev/null && echo "❌ setroubleshoot installed" || echo "✅ setroubleshoot not installed"
```
...
The target system uses **UKI (Unified Kernel Image) + direct UEFI boot with Secure Boot** rather than GRUB2.  This architectural decision provides stronger security than a traditional bootloader and exceeds the CIS and STIG requirements for bootloader hardening.
...
**Gentoo Implementation**
No GRUB configuration file exists.  The ESP (EFI System Partition) contains only signed `.efi` binaries (the UKIs).  The ESP is mounted at `/efi` with `noatime` and does not contain any plaintext configuration files that could reveal boot parameters or weaken security.
...
This section covers kernel parameters that harden process behaviour: address space layout randomisation (ASLR), ptrace restrictions, core dump restrictions, the NX/XD bit, kernel message buffer access, TCP syncookies, and the Ctrl‑Alt‑Delete handler.
...
**What's Required**
`kernel.randomize_va_space` must be set to `2` (full randomisation).  The STIG audit specifically checks for value `2`; some kernels support value `3`, which would cause a STIG audit failure despite being more secure.

**Gentoo Implementation**
The `cachyos‑sources` kernel defaults to `kernel.randomize_va_space=2`.  The UKI cmdline includes `mitigations=auto` which enables all relevant CPU vulnerability mitigations (README Part 8.2).
...
**What's Required**
`kernel.yama.ptrace_scope` must be set to `1`, `2`, or `3` to restrict the `ptrace()` system call.  Value `1` allows a process to trace only its descendants; this is the recommended setting for a development workstation where debugging is needed.  For stricter environments, value `2` (admin‑only) or `3` (completely disabled) may be used.

**Gentoo Implementation**
Set to `1` (restricted ptrace) via sysctl (README Part 1.5).
...
**What's Required**
Core dumps must be restricted via both `limits.conf` (`* hard core 0`) and the `fs.suid_dumpable` sysctl (`0`).  The `systemd‑coredump` service must also be configured to store nothing.

**Gentoo Implementation**
All three controls are implemented (README Part 1.5).  The `limits.conf` entry is in `/etc/security/limits.d/50-cis-core.conf`.  The sysctl is set via `/etc/sysctl.d/60-coredump.conf`.  The `systemd‑coredump` service has `Storage=none` and `ProcessSizeMax=0` in `/etc/systemd/coredump.conf.d/50-cis.conf`.
...
grep -E '^(Storage|ProcessSizeMax)=' /etc/systemd/coredump.conf /etc/systemd/coredump.conf.d/*.conf 2>/dev/null
# Storage should be 'none', ProcessSizeMax should be '0'
```
...
**What's Required**
The `prelink` package must not be installed, as it interferes with AIDE file integrity checking by modifying binaries.
...
**Gentoo Implementation**
Apport is an Ubuntu‑specific package and is not present on Gentoo.
...
**Gentoo Implementation**
Set via sysctl and enforced by the hardened Gentoo profile.
...
Warning banners inform users of their legal status regarding the system and must include the name of the organisation that owns the system and any monitoring policies.  The CIS benchmarks require that banners not display OS version information.
...
**Gentoo Implementation**
A custom warning banner is installed via README Part 25.  No OS version escapes are present.  The banner content is the standard CIS‑recommended warning text.
...
# Must reference a banner file (e.g., Banner /etc/issue.net)
```
...
**Gentoo Implementation**
The STIG provides a supplemental `ssh_confirm.sh` script that must be installed at `/etc/profile.d/ssh_confirm.sh`.
...
**Gentoo Implementation**
Automount and autorun are not applicable to a Wayland compositor that does not use GVFS/GDM.  USB storage is blacklisted at the kernel module level (see 1.1.1.10).  XDMCP is not present because the system uses Wayland, not X11.
...
The CIS benchmark lists 22 services that should be removed or masked if not required.  The target Gentoo system installs a minimal set of services; most CIS‑listed services are not present.  For each service, the audit checks whether it is installed and active; if a service is required for dependencies, it should be stopped and masked.
...
**What's Required**
The following services must not be installed or must be masked: `autofs`, `avahi`, `dhcp` (kea), `dns` (bind), `dnsmasq`, `ftp` (vsftpd), `ldap` (slapd), `dovecot`/`cyrus-imapd` (IMAP/POP3), `nfs-utils` (NFS server), `cups` (print server), `rpcbind`, `rsync-daemon`, `samba`, `net-snmp` (SNMP), `telnet-server`, `tftp-server`, `squid` (web proxy), `httpd`/`nginx` (web server), GDM (GNOME Display Manager), and `xorg-x11-server-Xwayland` (X Windows).

**Gentoo Implementation**
None of these services are installed on the target system.  The MTA is configured for local‑only delivery (see 2.1.21).

**Unified Audit**
```bash
#!/bin/bash
SERVICES=(autofs avahi-daemon kea named dnsmasq vsftpd slapd dovecot cyrus-imapd
    nfs-server cups rpcbind rsyncd smbd snmpd telnet tftp-server squid
    httpd nginx gdm xorg-x11-server-Xwayland)
for svc in "${SERVICES[@]}"; do
    state=$(systemctl is-active "$svc.service" 2>/dev/null || echo "inactive")
    [ "$state" = "active" ] && echo "❌ $svc is ACTIVE" || echo "✅ $svc inactive"
    qpkg -I "$svc" 2>/dev/null && echo "   ⚠️ $svc package installed" || true
done
```

**Remediation**
For any service found to be active:
```bash
systemctl stop <service>.service
systemctl mask <service>.service
# If the package is not needed for dependencies:
...
**Gentoo Implementation**
No traditional MTA (Postfix, Exim, Sendmail) is installed.  `msmtp` is used for outbound mail relay only (README Part 22.1).  It does not listen on any port.
...
**Gentoo Implementation**
Firewalld is configured with a default‑drop zone (see Section 4).  Only explicitly allowed ports are open: SSH on port 2222, DNS over TLS on 853, HTTPS on 443, and Cockpit on localhost:9090 (README Part 18.1).  OpenSnitch provides per‑application outbound control (README Part 18.2).
...
**What's Required**
The FTP client (`ftp`), LDAP client (`openldap-clients`), telnet client (`telnet`), TFTP client (`tftp`), and other insecure network clients must not be installed.
...
**Unified Audit**
```bash
for pkg in ftp openldap-clients telnet tftp rsh talk; do
...
**Remediation**
```bash
emerge --unmerge ftp openldap-clients telnet tftp rsh talk 2>/dev/null || true
```
...
All uncommon network protocol modules are blacklisted in `blacklist-hardening.conf` (README Part 16).  Each module requires both `install <module> /bin/true` and `blacklist <module>`.  After modifying the blacklist, the initramfs must be rebuilt.

---

##### 3.2.1 – 3.2.6 Ensure dccp, tipc, rds, sctp, atm, and can kernel modules are not available

**Profile:** CIS Level 2 – Server & Workstation | **Status:** ✅ COMPLIANT

**What's Required**
The following network protocol kernel modules must be blocked: `dccp` (Datagram Congestion Control Protocol), `tipc` (Transparent Inter‑Process Communication), `rds` (Reliable Datagram Sockets), `sctp` (Stream Control Transmission Protocol), `atm` (Asynchronous Transfer Mode), and `can` (Controller Area Network).

**Gentoo Implementation**
All six modules are blacklisted in `blacklist-hardening.conf` (README Part 16).

**Unified Audit**
```bash
for mod in dccp tipc rds sctp atm can; do
...
All network‑related kernel parameters from CIS §3.3 are enforced via a unified sysctl file at `/etc/sysctl.d/99-cis-network.conf` (README Part 3.3).  The table below lists every parameter, its expected value, and the CIS recommendation it satisfies.

| Parameter | Expected Value | CIS Recommendation |
|-----------|---------------|-------------------|
| `net.ipv4.ip_forward` | `0` | 3.3.1.1 |
| `net.ipv6.conf.all.forwarding` | `0` | 3.3.2.1 |
| `net.ipv6.conf.default.forwarding` | `0` | 3.3.2.2 |
| `net.ipv4.conf.all.send_redirects` | `0` | 3.3.1.4 |
| `net.ipv4.conf.default.send_redirects` | `0` | 3.3.1.5 |
| `net.ipv4.icmp_ignore_bogus_error_responses` | `1` | 3.3.1.6 |
| `net.ipv4.icmp_echo_ignore_broadcasts` | `1` | 3.3.1.7 |
| `net.ipv4.conf.all.accept_redirects` | `0` | 3.3.1.8 |
| `net.ipv4.conf.default.accept_redirects` | `0` | 3.3.1.9 |
| `net.ipv6.conf.all.accept_redirects` | `0` | 3.3.2.3 |
| `net.ipv6.conf.default.accept_redirects` | `0` | 3.3.2.4 |
| `net.ipv4.conf.all.secure_redirects` | `0` | 3.3.1.10 |
| `net.ipv4.conf.default.secure_redirects` | `0` | 3.3.1.11 |
| `net.ipv4.conf.all.rp_filter` | `1` | 3.3.1.12 |
| `net.ipv4.conf.default.rp_filter` | `1` | 3.3.1.13 |
| `net.ipv4.conf.all.accept_source_route` | `0` | 3.3.1.14 |
| `net.ipv4.conf.default.accept_source_route` | `0` | 3.3.1.15 |
| `net.ipv6.conf.all.accept_source_route` | `0` | 3.3.2.5 |
| `net.ipv6.conf.default.accept_source_route` | `0` | 3.3.2.6 |
| `net.ipv4.conf.all.log_martians` | `1` | 3.3.1.16 |
| `net.ipv4.conf.default.log_martians` | `1` | 3.3.1.17 |
| `net.ipv4.tcp_syncookies` | `1` | 3.3.1.18 |
| `net.ipv6.conf.all.accept_ra` | `0` | 3.3.2.7 |
| `net.ipv6.conf.default.accept_ra` | `0` | 3.3.2.8 |

**Status:** ✅ COMPLIANT for all parameters.

**Audit (spot‑check key parameters)**
```bash
...
**Remediation (if any parameter differs)**
```bash
...
## 4. Host‑Based Firewall — Firewalld

**Source: CIS AlmaLinux OS 10 Benchmark v1.0.0, Section 4.1**

The target system uses **firewalld** with the **nftables** backend (README Part 18.1).  The recommendations in this section are taken directly from the CIS AlmaLinux OS 10 Benchmark, which provides native firewalld‑specific audit and remediation procedures.  On Gentoo, firewalld uses nftables as its default backend; unlike RPM‑based distributions, no additional backend configuration is required beyond the `FirewallBackend` setting in `/etc/firewalld/firewalld.conf`.

The AlmaLinux benchmark is organised as follows:
- **4.1.1** — Ensure firewalld is installed.
- **4.1.2** — Ensure the firewalld backend is configured (must be `nftables`).
- **4.1.3** — Ensure `firewalld.service` is running and enabled.
- **4.1.4** — Ensure the active zone target is not set to `ACCEPT` and that the target is written as a permanent rule.
- **4.1.5** — Ensure loopback traffic is configured (the `lo` interface must be in a zone with target `ACCEPT`).
- **4.1.6** — Ensure anti‑spoofing rich rules are configured for loopback source addresses.
- **4.1.7** — Ensure only approved services and ports are permitted.

Each subsection below reproduces the AlmaLinux benchmark's audit procedure (adapted for Gentoo's filesystem layout and command conventions) and its remediation procedure.
...
**What's Required**
The `firewalld` package must be installed.  `firewalld` provides a dynamically managed firewall with support for network/firewall zones that define the trust level of network connections or interfaces.  It acts as a front‑end for the Linux kernel's netfilter framework via the nftables userspace utility.

**Gentoo Implementation**
`net-firewall/firewalld` is emerged and active (README Part 18.1).  The package is installed from the Gentoo main repository.

**Audit**
```bash
qpkg -I firewalld && echo "✅ firewalld installed" || echo "❌ firewalld NOT installed"
```

If the command returns nothing (package not installed), the audit fails.
...
**What's Required**
The `FirewallBackend` parameter in `/etc/firewalld/firewalld.conf` must be set to `nftables`.  The `iptables` backend is deprecated and should not be used.  On Gentoo, nftables is the default and only supported backend for modern firewalld deployments.

**Gentoo Implementation**
The configuration file already contains `FirewallBackend=nftables` (README Part 18.1).  This is the default setting on Gentoo and on any modern firewalld installation.

**Audit**
```bash
grep -Psi -- '^\h*FirewallBackend\h*=\h*nftables\b' /etc/firewalld/firewalld.conf
```

The command must return a matching line showing `FirewallBackend=nftables`.  If it returns nothing, or if the line is commented out or set to a different value, the audit fails.

**Remediation**
```bash
# If the line exists but has a different value:
...
**What's Required**
`firewalld.service` must be enabled to start automatically at boot and must currently be active.  The firewall rules configured through FirewallD are only enforced when the service is running.

**Gentoo Implementation**
`firewalld.service` is enabled and active (README Part 18.1).
...
If either command returns anything other than `enabled` or `active` respectively, the audit fails.
...
**What's Required**
For every active zone except those containing only the loopback interface (`lo`), the zone target must **not** be set to `ACCEPT`.  Additionally, the target must be written as a permanent rule (i.e., committed with `--permanent`) so that it survives a daemon reload or system reboot.  This implements a "deny by default, allow by exception" policy: any traffic not explicitly permitted by a service, port, or rich rule is dropped or rejected.

The AlmaLinux benchmark notes that the target may be set to `DROP`, `REJECT`, or `default`.  The `default` target provides a convenient way to implement a deny‑by‑default policy while still allowing essential ICMP traffic (such as neighbour discovery and router advertisements).

**Gentoo Implementation**
The default zone is `drop`, and the active interfaces (Ethernet) are assigned to the `drop` zone.  The loopback interface is in the `trusted` zone, which has target `ACCEPT` — this is explicitly permitted by the AlmaLinux benchmark (see 4.1.5).  All zone assignments are permanent.

The audit script from the AlmaLinux benchmark has been adapted below to run on Gentoo.  It performs the following checks:
1. Iterates over all active zones.
2. Excludes zones whose only interface is `lo` or virtual interfaces (`virbr*`).
3. For each remaining zone, extracts the running target and the permanent target.
4. If the running target is unset or `ACCEPT`, the zone fails.
5. If the running target does not match the permanent target, the zone fails (the rule is not permanent).

**Audit**
```bash
#!/bin/bash
# Adapted from CIS AlmaLinux OS 10 Benchmark §4.1.4
a_output=(); a_output2=()
while IFS= read -r l_zone; do
    l_zone="$(awk -F'(' '{print $1}' <<< "${l_zone// /}")"
    l_interfaces="$(firewall-cmd --zone="$l_zone" --list-interfaces)"
    # Exclude loopback and virtual interfaces from the ACCEPT check
...
if [ "${#a_output2[@]}" -gt 0 ]; then
    printf '%s\n' "" "${a_output2[@]}" ""
    echo "❌ Some zones have an incorrect or non‑permanent target"
else
    echo "✅ All active zones have a valid, permanent target set"
fi
```

No lines should be printed by the `a_output2` array.  If `a_output2` is empty, the system is compliant.

**Remediation**
```bash
# Set the target for a specific zone and make it permanent.
# Replace <ZONE> with the zone name (e.g., 'drop', 'public') and
# <TARGET> with the desired target (DROP, REJECT, or default).
firewall-cmd --permanent --zone=<ZONE> --set-target=<TARGET>
firewall-cmd --reload

# Example: set the 'drop' zone to DROP
...
**What's Required**
Loopback traffic (communications between processes on the same machine) must be permitted.  The loopback interface (`lo`) must be assigned to a zone whose target is `ACCEPT`.  If the loopback interface is not assigned to any zone (the output of `firewall-cmd --get-zone-of-interface=lo` is `no zone`), the audit fails immediately.

The AlmaLinux benchmark requires two checks:
1. Verify that `lo` is assigned to a zone (not `no zone`).
2. Verify that the assigned zone's target is `ACCEPT`.

**Gentoo Implementation**
The `lo` interface is in the `trusted` zone, which has the target `ACCEPT` (README Part 18.1).  This is consistent with the AlmaLinux benchmark's expectation that loopback traffic should not be restricted.

**Audit**
```bash
# Step 1: Verify that lo is assigned to a zone
ZONE=$(firewall-cmd --get-zone-of-interface=lo)
if [ "$ZONE" = "no zone" ]; then
    echo "❌ lo is not assigned to any zone"
    exit 1
fi
echo "lo is in zone: $ZONE"

# Step 2: Verify that the zone's target is ACCEPT
firewall-cmd --list-all --zone="$ZONE" | grep -Psi -- '^\h*target:\h+\H+'
# The output must show "target: ACCEPT"
```

Both steps must succeed.  If the target is anything other than `ACCEPT`, or if `lo` is not assigned to a zone, the audit fails.

**Remediation**
If the loopback interface is not already assigned to a zone:
```bash
firewall-cmd --permanent --zone=trusted --add-interface=lo
```
If it is in a different zone and needs to be moved:
```bash
firewall-cmd --permanent --zone=trusted --change-interface=lo
```
Then reload the firewall to apply the change immediately:
```bash
firewall-cmd --reload
```
...
### 4.1.6 Ensure firewalld loopback source address traffic is configured (Manual)

**Profile:** CIS Level 1 – Server & Workstation | **Status:** ✅ COMPLIANT

**What's Required**
Rich rules must be present that drop any traffic **from** a loopback source address (`127.0.0.1` or `::1`) that is **not also destined to** a loopback address.  This is an anti‑spoofing measure: the loopback interface is the only place where loopback network traffic should appear; all other interfaces should ignore traffic with these source addresses.

The AlmaLinux benchmark requires that the following two rich rules be present on the zone that contains the `lo` interface:
...
**Gentoo Implementation**
These rich rules are already configured on the `trusted` zone (README Part 18.1).

**Audit**
```bash
#!/bin/bash
# Adapted from CIS AlmaLinux OS 10 Benchmark §4.1.6
while IFS= read -r l_zone; do
    echo " - Zone: $l_zone"
...
Both rules (one for IPv4, one for IPv6) must be present.  If either rule is missing, the audit fails.

**Remediation**
If the rules are missing, add them to the zone that contains the loopback interface.  The AlmaLinux benchmark provides the following commands (adapted for Gentoo):

```bash
# Determine the zone that contains lo
LO_ZONE=$(firewall-cmd --get-zone-of-interface=lo)

# Add the IPv4 anti‑spoofing rule
firewall-cmd --permanent --zone="$LO_ZONE" --add-rich-rule='rule family=ipv4 source address="127.0.0.1" destination not address="127.0.0.1" drop'

# Add the IPv6 anti‑spoofing rule
firewall-cmd --permanent --zone="$LO_ZONE" --add-rich-rule='rule family=ipv6 source address="::1" destination not address="::1" drop'
...
**Profile:** CIS Level 1 – Server & Workstation | **Status:** ⚠️ NEEDS REVIEW (site‑specific)

**What's Required**
The services and ports permitted in each active firewall zone must be reviewed against site policy.  Every allowed service or port should have a documented business need.  The default zone target (DROP or REJECT, as configured in 4.1.4) ensures that all traffic not explicitly permitted is denied.

The AlmaLinux benchmark notes that the command `firewall-cmd --get-services | tr ' ' '\n' | column` can be used to list all services that firewalld recognises by name.

**Gentoo Implementation**
The user's firewalld configuration (README Part 18.1) allows the following services and ports in the `drop` zone:
- SSH on port 2222 (TCP)
- DNS over TLS on port 853 (TCP)
- HTTPS on port 443 (TCP and UDP)
- Cockpit on port 9090 (TCP, restricted to `127.0.0.1` only)

All other inbound traffic is dropped by the zone's default DROP target.  The administrator must verify that this list conforms to local site policy.

**Audit**
```bash
# List the services and open ports for the active zone(s)
firewall-cmd --list-all --zone="$(firewall-cmd --list-all | awk '/(active)/ { print $1 }')" | \
  grep -P -- '^\h*(services:|ports:)'

# Also review rich rules (which may open additional ports)
firewall-cmd --list-rich-rules
```

Review the output.  Every service and port listed should have a documented justification.  Any service or port that is not required should be removed.
...
Remove an unnecessary port:
```bash
firewall-cmd --permanent --remove-port=<port-number>/<port-type>
# Example: firewall-cmd --permanent --remove-port=25/tcp
```

Make the running configuration permanent and reload:
```bash
...
Secure Shell (SSH) is the primary remote administration protocol on the target system.  The SSH daemon configuration (README Part 19.7) satisfies or exceeds every CIS Level 1 SSH recommendation.  The SSH client configuration (README Part 19.5) applies matching hardening on the client side.  Both configurations are reproduced in full below, with individual audit commands for each parameter.

The OpenSSH daemon reads its configuration from `/etc/ssh/sshd_config` and from files in `/etc/ssh/sshd_config.d/*.conf` (via the `Include` directive).  On Gentoo, the system‑wide crypto policy is applied through direct configuration in `sshd_config` rather than through a separate crypto‑policies framework.  The `apparmor.d` project ships a mature `sshd` profile that should be loaded in enforce mode.
...
**What's Required**
The file `/etc/ssh/sshd_config` and any files in `/etc/ssh/sshd_config.d/` ending in `.conf` must be owned by `root:root` and have mode `0600` or more restrictive.

**Audit**
```bash
stat -c '%a %U:%G' /etc/ssh/sshd_config
# Must show: 600 root:root
find /etc/ssh/sshd_config.d -type f -name '*.conf' -exec stat -c '%a %U:%G %n' {} \;
# All files must show 600 root:root (or more restrictive)
```
...
**What's Required**
Private SSH host key files must be owned by `root:root` (or `root:ssh_keys` on Fedora‑derived distributions) and have mode `0600` or `0640` depending on the group owner.  Public SSH host key files must be owned by `root:root` and have mode `0644` or more restrictive.

**Gentoo Implementation**
On Gentoo, the `ssh_keys` group does not exist.  Private keys are owned by `root:root` with mode `0600`.  Public keys are owned by `root:root` with mode `0644`.
...
**Remediation**
```bash
find /etc/ssh -type f -name 'ssh_host_*_key' ! -name '*.pub' -exec chown root:root {} \; -exec chmod 600 {} \;
...
# Ensure the sshd_config contains:
# AllowGroups sshusers
```
...
**Profile:** CIS Level 1 – Server & Workstation · STIG CAT II | **Status:** ⚠️ NEEDS REVIEW (STIG banner content)

**What's Required**
The `Banner` directive must point to a file whose contents are displayed to the remote user before authentication is permitted.  The STIG requires the exact DoD‑mandated banner text.

**Gentoo Implementation**
`Banner /etc/ssh/banner` is configured.  The banner content must be reviewed for STIG compliance (see §1.6).

**Audit**
```bash
sshd -T | grep -i '^banner '
# Must output a path, e.g., banner /etc/ssh/banner
cat "$(sshd -T | awk '/^banner / {print $2}')"
```
...
**Gentoo Implementation**
The target system uses `chacha20-poly1305@openssh.com, aes256-gcm@openssh.com, aes256-ctr` (README Part 19.7).  ChaCha20‑Poly1305 is **not** FIPS‑approved but is **cryptographically stronger** than AES‑128 variants: it is constant‑time on all CPUs, provides better security on machines without AES‑NI, and is recommended by the cryptographic community.  This is a deliberate, risk‑informed deviation (see §8).
...
| `kexalgorithms` | Post‑quantum hybrids (see §8) | 5.1.12 | UBTU‑24‑100840 |
| `logingracetime` | `30` | 5.1.13 | – |
| `loglevel` | `VERBOSE` | 5.1.14 | – |
| `macs` | `hmac‑sha2‑512‑etm@openssh.com, hmac‑sha2‑256‑etm@openssh.com` | 5.1.15 | UBTU‑24‑100830 |
| `maxauthtries` | `3` | 5.1.16 | – |
| `maxsessions` | `3` | 5.1.17 | – |
| `maxstartups` | `10:30:60` | 5.1.18 | – |
...
**Status:** ✅ COMPLIANT for all parameters except where intentional deviations are documented.

---

### 5.2 Configure Privilege Escalation — sudo

The `sudo` utility provides fine‑grained control over privileged command execution.  All CIS §5.2 sudo recommendations are satisfied on the target system.  The sudo configuration is managed through `/etc/sudoers` and files in `/etc/sudoers.d/`; all edits must be performed with `visudo` to prevent syntax errors.

---

##### 5.2.1 – 5.2.7 Quick Reference: All sudo Parameters

| CIS ID | Requirement | Gentoo Implementation | Audit Command | STIG ID |
|--------|-------------|----------------------|---------------|---------|
| 5.2.1 | sudo installed | `app-admin/sudo` emerged | `which sudo` | – |
| 5.2.2 | Commands use pty | `Defaults use_pty` in sudoers | `grep 'use_pty' /etc/sudoers /etc/sudoers.d/*` | – |
| 5.2.3 | Log file exists | `Defaults logfile="/var/log/sudo.log"` | `grep 'logfile' /etc/sudoers /etc/sudoers.d/*` | – |
| 5.2.4 | Password required (L2) | No `NOPASSWD` tags | `grep -r 'NOPASSWD' /etc/sudoers /etc/sudoers.d/` | – |
| 5.2.5 | Re‑authentication not disabled | No `!authenticate` tags | `grep -r '!authenticate' /etc/sudoers /etc/sudoers.d/` | UBTU‑24‑300021 |
| 5.2.6 | Authentication timeout ≤ 15 min | `Defaults timestamp_timeout=15` | `sudo -V \| grep 'Authentication timestamp timeout'` | – |
| 5.2.7 | su command restricted | `pam_wheel.so` in `/etc/pam.d/su` | `grep 'pam_wheel.so' /etc/pam.d/su` | – |
...
The PAM stack on the target system is configured in `/etc/pam.d/system-auth` (README Part 20).  Gentoo does not use a PAM configuration manager; the stack files are edited directly.  The key modules — `pam_faillock`, `pam_pwquality`, `pam_pwhistory`, and `pam_unix` — are configured via dedicated configuration files in `/etc/security/`.

---

##### 5.3.1.1 Ensure libpam‑pwquality is installed
...
All PAM arguments are configured via `faillock.conf`, `pwquality.conf`, and the PAM stack files (README Part 20).  The following table shows the key STIG‑mapped settings:

| Setting | Configuration File | CIS Recommended Value | STIG Required Value | Gentoo Value | Status |
|---------|-------------------|----------------------|---------------------|--------------|--------|
...
| `even_deny_root` | `faillock.conf` | Level 2 only | Required | `even_deny_root` | ✅ |
...
| `enforce_for_root` | `pwquality.conf` | Enabled | Enabled | `enforce_for_root` | ✅ |
| Hash algorithm | `pam_unix.so` | `sha512` or `yescrypt` | `SHA512` | `yescrypt` | ✅ (stronger) |
| `remember` | `pwhistory.conf` | ≥ 24 | N/A | `remember = 24` | ✅ |
...
All CIS §5.4 recommendations are satisfied on the target system.  The following table lists the key settings:

| Setting | Configuration File | Gentoo Value | CIS ID | STIG ID |
|---------|-------------------|-------------|--------|---------|
...
**Status:** ✅ COMPLIANT for all settings.

**Audit (password aging)**
```bash
...
```bash
passwd -S root
# Must show: root L ... (locked)
```

**Audit (TMOUT)**
```bash
grep -r 'TMOUT=' /etc/profile.d/ /etc/profile /etc/bashrc 2>/dev/null
```
...
### 6.1 Configure System Logging — systemd‑journald

The target system uses **systemd‑journald** as its sole logging system (README Part 6).  `rsyslog` is not installed.  Journald is configured for persistent storage with compression and size‑based rotation.

---

##### 6.1.1.1 Ensure journald service is active
...
**Gentoo Implementation**
The target system uses `systemd‑journald` with persistent storage.  Journald provides equivalent logging functionality with structured, indexed binary logs.  The `audit` subsystem captures authentication and privilege escalation events (see §6.2).  If STIG compliance is legally required, `rsyslog` can be emerged alongside journald.
...
The audit subsystem is configured via `/etc/audit/auditd.conf` (daemon configuration) and `/etc/audit/rules.d/99-hardening.rules` (ruleset) as described in README Part 15.  The comprehensive ruleset covers all CIS §6.2 and STIG audit requirements.  On Gentoo, `augenrules` is enabled via `/etc/conf.d/auditd` (the `USE_AUGENRULES="yes"` setting), which merges component rule files from `/etc/audit/rules.d/` into `/etc/audit/audit.rules` at service startup.

---

##### 6.2.1.1 Ensure auditd packages are installed

*STIG: UBTU‑24‑100400 (CAT II)*

**Profile:** CIS Level 2 – Server & Workstation · STIG CAT II | **Status:** ✅ COMPLIANT

**Audit**
```bash
qpkg -I audit && echo "✅ audit installed" || echo "❌ audit NOT installed"
```
...
# Both must output 'enabled' and 'active' respectively.
```
...
*STIG: UBTU‑24‑102010 (CAT II)*

**Profile:** CIS Level 2 – Server & Workstation · STIG CAT II | **Status:** ✅ COMPLIANT

**What's Required**
The kernel parameter `audit=1` must be present on the kernel command line so that processes starting before the audit daemon are still auditable.

**Gentoo Implementation**
`audit=1` is embedded in the UKI command line (README Part 8.2).
...
**Remediation**
Add `audit=1` to `/etc/kernel/cmdline` and rebuild the UKI.
...
**What's Required**
The `audit_backlog_limit` kernel parameter must be set to a value sufficient to prevent audit event loss during boot (recommended: 8192 or larger).

**Gentoo Implementation**
`audit_backlog_limit=8192` is embedded in the UKI command line (README Part 8.2).

**Audit**
```bash
grep -o 'audit_backlog_limit=[0-9]*' /proc/cmdline
```

---

##### 6.2.2.1 – 6.2.2.4 Data Retention Configuration

All four retention settings are configured in `/etc/audit/auditd.conf` (README Part 15.2):

| Parameter | Gentoo Value | Description |
|-----------|-------------|-------------|
| `max_log_file` | `50` | Maximum audit log file size in megabytes |
| `max_log_file_action` | `KEEP_LOGS` | Do not automatically delete old logs |
| `disk_full_action` | `HALT` | Halt the system if the audit partition is full |
| `disk_error_action` | `SYSLOG` | Log a warning if a disk error is detected |
| `space_left_action` | `SYSLOG` | Warn when disk space is low |
| `admin_space_left_action` | `HALT` | Halt when administrative space threshold is reached |

**Status:** ✅ COMPLIANT for all retention settings.
...
##### 6.2.3.1 – 6.2.3.21 Audit Rules

The full audit ruleset is in `/etc/audit/rules.d/99-hardening.rules` (README Part 15.3).  This covers all CIS §6.2.3 and STIG audit rules.  Key rule categories include:

| Category | Rule Key(s) | Purpose |
|----------|------------|---------|
| File integrity monitoring | `etc_changes`, `identity`, `pam_config`, `sudoers_change`, `sshd_config` | Detect modifications to critical configuration files |
| Privileged command execution | `setuid_exec`, `setgid_exec`, `sudo_exec`, `su_exec` | Log use of setuid/setgid binaries and privilege escalation |
| Authentication events | `login_logout`, `failed_login`, `session_tracking` | Track successful and failed authentication attempts |
| Network socket creation | `socket_ipv4`, `socket_ipv6`, `network_connect` | Monitor network connections from user processes |
| Kernel module loading | `module_load`, `module_unload`, `kmod_exec` | Detect loading and unloading of kernel modules |
| User/group management | `user_mgmt_exec`, `perm_change`, `owner_change` | Audit changes to user accounts and file ownership |
| Time modification | `time_change`, `timezone_change` | Detect changes to system clock and timezone |

The ruleset is locked (immutable) via the `-e 2` directive after thorough testing.
...
**Remediation (if the rules are not loaded)**
```bash
augenrules --load
# If immutable mode is active and rules have changed, a reboot is required:
auditctl -s | grep "enabled" | grep -q "2" && echo "Reboot required to load new rules"
```

---

##### 6.2.4.1 – 6.2.4.10 Audit File Access Permissions
...
| Path | Expected Mode | Expected Owner:Group |
|------|-------------|---------------------|
| `/etc/audit/auditd.conf` | `0640` | `root:root` |
| `/etc/audit/audit.rules` | `0640` | `root:root` |
| `/etc/audit/rules.d/*.rules` | `0640` | `root:root` |
| `/var/log/audit/` (directory) | `0750` | `root:root` |
| `/var/log/audit/audit.log` | `0600` | `root:root` |
| `/sbin/auditctl` | `0755` | `root:root` |
| `/sbin/ausearch` | `0755` | `root:root` |
| `/sbin/aureport` | `0755` | `root:root` |
| `/sbin/autrace` | `0755` | `root:root` |
| `/sbin/auditd` | `0755` | `root:root` |
| `/sbin/augenrules` | `0755` | `root:root` |
...
AIDE (Advanced Intrusion Detection Environment) provides cryptographic file integrity monitoring.  It computes SHA‑256 checksums of critical system files and compares them against a known‑good baseline database.
...
**What's Required**
AIDE must run regularly (at least weekly).  The database must be updated after legitimate system changes.

**Gentoo Implementation**
A weekly AIDE check is integrated into the `weekly-security-scan.timer` and its associated script (README Part 18.4.3).
...
**Gentoo Implementation**
The AIDE configuration must include specific rules for audit tools with `sha512`.  The default AIDE configuration may use `sha256`; this must be verified or updated.
...
All CIS §7.1 file permission recommendations are satisfied on the target system.  The following table lists each file and its expected permissions:
...
| `/etc/shadow` | `0000` | `root:root` | 7.1.5 |
| `/etc/shadow-` | `0000` | `root:root` | 7.1.6 |
| `/etc/gshadow` | `0000` | `root:root` | 7.1.7 |
| `/etc/gshadow-` | `0000` | `root:root` | 7.1.8 |
| `/etc/shells` | `0644` | `root:root` | 7.1.9 |
| `/etc/security/opasswd` | `0600` | `root:root` | 7.1.10 |

**Status:** ✅ COMPLIANT for all files.
...
**Remediation (for any file with incorrect permissions)**
```bash
...
##### 7.1.11 – 7.1.13 World‑writable files, unowned files, and SUID/SGID review

**Profile:** CIS Level 1 – Server & Workstation | **Status:** ✅ COMPLIANT

**What's Required**
- No world‑writable files should exist on the local filesystem.
- All world‑writable directories must have the sticky bit set.
- No files or directories should exist without a valid owner and group.
- All SUID and SGID executables must be reviewed and justified.

**Gentoo Implementation**
The system is checked for these conditions as part of regular security scans.  Any anomalies are investigated and remediated.

**Audit (world‑writable files)**
```bash
find / -xdev -type f -perm -0002 ! -path "/proc/*" ! -path "/sys/*" ! -path "/run/*" 2>/dev/null
```

**Audit (world‑writable directories without sticky bit)**
```bash
find / -xdev -type d -perm -0002 ! -perm -1000 ! -path "/proc/*" ! -path "/sys/*" 2>/dev/null
```
...
```bash
find / -xdev -type f \( -perm -4000 -o -perm -2000 \) ! -path "/proc/*" ! -path "/sys/*" 2>/dev/null
```
...
All CIS §7.2 recommendations are satisfied.  Key checks:
...
- **7.2.4–7**: No duplicate UIDs, GIDs, usernames, or group names.
- **7.2.8**: All local interactive user home directories exist, are owned by the user, and have mode `0750` or more restrictive.
- **7.2.9**: User dot files (`.forward`, `.rhost`, `.netrc`, `.bash_history`) have correct permissions.

**Status:** ✅ COMPLIANT for all checks.
...
cut -f3 -d: /etc/group | sort -n | uniq -d | while read gid; do
    echo "Duplicate GID $gid: $(awk -F: -v g=$gid '$3==g{print $1}' /etc/group | xargs)"
done
```
...
The target system's cryptographic choices are optimised for the APT (Advanced Persistent Threat) threat model rather than for FIPS compliance.  The following table documents each intentional deviation:
...
| Password hashing (PAM) | yescrypt | SHA‑512 | yescrypt is memory‑hard; recommended by systemd upstream |
| SSH encryption | ChaCha20‑Poly1305 | AES‑128‑GCM | Constant‑time on all CPUs; no AES‑NI dependency |
| SSH key exchange | sntrup761x25519‑sha512, mlkem768x25519‑sha256 | ECDH NIST P‑256/384/521 | Post‑quantum hybrid; quantum‑safe |

**FIPS mode** (`fips=1` on the kernel command line) is **not enabled**.  FIPS is a compliance standard, not a security standard.  Enabling it would require downgrading several cryptographic choices, weakening the system against the nation‑state APT threat model.
...
**What the STIG Requires**
The system must run in FIPS mode and use only FIPS‑validated cryptographic modules.

**Gentoo Implementation**
FIPS mode is not enabled.  See rationale above.

**Remediation (if FIPS compliance is legally required)**
```bash
# Add fips=1 to /etc/kernel/cmdline and rebuild UKI
...
**What's Required**
All persistent disk partitions must be encrypted at

...
### 6.1 Install Limine and AUR Helper
...
## Part 7: Setup Snapper for Bootable Snapshots

### 7.1 Install Snapper and Dependencies

```bash
pacman -S snapper inotify-tools jdk-openjdk
```

### 7.2 Configure Snapper
...
mkdir /.snapshots
mount -o noatime,compress=zstd:3,space_cache=v2,discard=async,subvol=@snapshots /dev/mapper/vg0-root /.snapshots
...
ROOT_SUBVOL_ID=$(btrfs subvolume list / | grep "path @$" | awk '{print $2}')
echo "Root subvolume ID: $ROOT_SUBVOL_ID"
btrfs subvolume set-default ${ROOT_SUBVOL_ID} /
```

### 7.3 Configure Snapper Settings
...
### 7.4 Install snap-pac for Automatic Snapshots
...
## Part 8: Setup Limine-Snapper Integration

### 8.1 Install limine-snapper-sync

```bash
# Install from AUR
# Temporarily allow passwordless sudo
echo "ahsan ALL=(ALL:ALL) NOPASSWD: ALL" >> /etc/sudoers.d/temp_install
chmod 440 /etc/sudoers.d/temp_install

sudo -u ahsan paru -S limine-snapper-sync --noconfirm

# Remove temporary passwordless sudo
rm /etc/sudoers.d/temp_install
```

### 8.2 Configure limine-snapper-sync

```bash
cat > /etc/default/limine-snapper-sync << 'EOF'
### OS Entry Targeting
...
### 8.3 Enable limine-snapper-sync Service
...
## Part 9: Install CachyOS Kernel (Optional but Recommended)
...
## Part 10: Final System Configuration

### 10.1 Enable Essential Services

```bash
systemctl enable NetworkManager fstrim.timer
```

### 10.2 Create Initial Baseline Snapshot

```bash
# Create a baseline snapshot after installation
snapper -c root create --description "Fresh Installation - Baseline"
...
## Post-Installation: First Boot and Testing
...
# Reboot to see the snapshot in Limine menu
sudo reboot
...
snapper info <snapshot-number>

# Compare snapshots
snapper diff <snapshot1> <snapshot2>
```
...
```bash
# Check dracut config
cat /etc/dracut.conf.d/myconf.conf
...
You now have a custom Arch Linux installation that works exactly like CachyOS:

✅ **Full disk encryption** (LUKS2 + LVM)  
✅ **Optimized Btrfs** with zstd:3 compression  
✅ **Dracut initramfs** (required for bootable snapshots)  
✅ **Fast Limine bootloader**  
✅ **Automatic snapshots** before every pacman operation  
✅ **Bootable snapshots** in Limine menu  
✅ **Easy rollbacks** from snapshots (4 different methods)  
✅ **CachyOS repositories** with optimized packages  
✅ **CachyOS kernel** (optional)

### Key Differences from Standard Arch:

- Uses **dracut** instead of mkinitcpio
- Uses **limine-dracut** command instead of `mkinitcpio -P`
- Kernel parameters use **dracut syntax** (`rd.luks.name` instead of `cryptdevice`)
- **Automatic** snapshot management via limine-snapper-sync
- **CachyOS optimized** packages and kernel

Your system will automatically create snapshots before updates and show them in the boot menu for instant rollback!

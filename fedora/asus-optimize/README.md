...
**On Fedora 43 (kernel 6.18)**, the ASUS power limit sysfs interface for PPT values lives at the legacy platform path:
...
| `asusd` | Loaded by udev. The boot unit is ordered to run after it |
| `power-profiles-daemon` | Loaded at boot. The boot unit is ordered to run after it |
...
# Kernel 6.19+ (after upgrade — PPT only; throttle_thermal_policy stays at legacy path)
...
On kernel 6.18, accessing the legacy PPT sysfs path causes a one-time kernel log message:
